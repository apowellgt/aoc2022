...
# and returns True if the two ranges overlap, and False otherwise.
def range_overlaps(range1, range2):
  ...
  # Check if the start and end of one range fall within the other range.
...
counter = 0
for pair in pairs:
  if range_overlaps(pair[0], pair[1]) or range_overlaps(pair[1], pair[0]):
    counter += 1
